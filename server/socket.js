...
const setupSocket = (server) => {
    ...
    const sendMessage = async(message) => {
        ...
        // Fetch user information to get the first names
        const senderUser = await User.findById(message.sender).select("firstName");
        const recipientUser = await User.findById(message.recipient).select("firstName");

        // Create the message with first names
        const createdMessage = await Message.create({
            ...message,
            senderFirstName: senderUser.firstName, // Add sender's first name
            recipientFirstName: recipientUser.firstName, // Add recipient's first name
        });
